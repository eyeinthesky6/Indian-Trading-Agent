...
def derive_long_swing_setup(snapshot: dict, regime: dict) -> dict:
    ...
    moving its own threshold upward.
    """
    ...
    if direction == "trending_up":
        near_prior_high = latest >= prior_high20 * 0.98
        if near_prior_high:
            setup_name = "prior-20-session breakout continuation"
            entry_low = prior_high20
            entry_high = prior_high20 + max(0.25 * atr, prior_high20 * 0.002)
            ...
            entry_low = max(sma20 - 0.35 * atr, prior_low20)
            entry_high = sma20 + 0.35 * atr
            stop = min(prior_low20 - 0.25 * atr, sma20 - 1.5 * atr)
            rationale.extend([
        ...
        stop = min(entry_low - 1.25 * atr, prior_low20 - 0.25 * atr)
        ...
        entry_low = prior_high20
        entry_high = prior_high20 + max(0.25 * atr, prior_high20 * 0.002)
        stop = min(sma20 - 0.5 * atr, prior_high20 - 1.5 * atr)
        entry_condition = "cross_above_zone"
        rationale.extend([
